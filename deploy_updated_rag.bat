@echo off
REM Deploy updated RAG enablement to Docker container

REM Set variables
set CONTAINER_NAME=farmlore-web-1
set SCRIPT_FILE=updated_rag_enablement.py
set CONTAINER_PATH=/app/

echo Deploying updated RAG enablement to Docker container...
...
echo Copying updated RAG enablement script to container...
docker cp %SCRIPT_FILE% %CONTAINER_NAME%:%CONTAINER_PATH%

REM Execute the script in the container
echo Executing the updated RAG enablement script in the container...
docker exec %CONTAINER_NAME% python /app/updated_rag_enablement.py

REM Check if the script executed successfully
if errorlevel 1 (
  echo Error: The updated RAG enablement script failed to execute successfully.
  exit /b 1
)
...
echo.
echo =========================================================
echo Updated RAG enablement deployment complete! Container has been restarted.
echo.
