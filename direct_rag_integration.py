...
def test_direct_rag():
    ...
    print("=== Direct RAG Integration Test ===")
    
    test_query = "How do I control aphids on tomatoes?"
    print(f"Testing with query: '{test_query}'")
    
    # Simulate an original response
    original_response = "To control aphids on tomatoes, you can try using insecticidal soap or neem oil spray."
...
# Create function to integrate directly with HybridEngine
def enhance_hybrid_engine_response(hybrid_engine, params: Dict[str, Any], response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        hybrid_engine: The HybridEngine instance
        params: The query parameters
        response: The original response from the engine
    ...
    # Skip enhancement for empty queries or responses
    if not query or not original_text:
        return response
    
    # Enhance the response
    enhanced_text, results = enhance_response(query, original_text)
    
    if results:
        logger.info(f"Enhanced response with information from: {results[0]['title']}")
        
        # Add information about the RAG sources
