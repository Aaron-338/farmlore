#!/usr/bin/env python
"""
Patched Standalone RAG Module

This is a fixed version of the standalone RAG module with improved matching for tomato queries.
"""
import re
...
def search_pest_data_improved(query: str, top_n: int = 2) -> List[Dict[str, Any]]:
    """IMPROVED search for pest data directly using keyword matching and text similarity"""
    query_keywords = set(get_keywords(query))
    results = []
    
    # Get plant type context from the query
    query_lower = query.lower()
    plant_context = None
    
    # Define plant types to check for
    plant_types = {
    ...
    for item in PEST_DATA:
        ...
        # Base combined score (as in original function)
        score = (
        ...
        # Apply plant context bonus if both query and item match the same plant
        if plant_context:
            item_title_lower = title.lower()
            item_plant_match = False
            
            # Check if the title contains the plant context
            for keyword in plant_types[plant_context]:
                if keyword in item_title_lower:
                    item_plant_match = True
                    break
            
            # Apply bonus to matching plant context
            if item_plant_match:
                score += 3.0  # Significant bonus for matching plant context
...
def enhance_response(query: str, original_response: str) -> str:
    ...
    try:
        # Search for relevant information using improved search
...
if __name__ == "__main__":
    ...
    # Test with original search function
    print("\nOriginal search function results:")
    original_results = search_pest_data(test_query)
    print(f"Top match: {original_results[0]['title']} (Score: {original_results[0]['score']:.2f})")
    
    # Test with improved search function
    print("\nImproved search function results:")
    improved_results = search_pest_data_improved(test_query)
    print(f"Top match: {improved_results[0]['title']} (Score: {improved_results[0]['score']:.2f})")
    
    # Test the enhanced response with improved search
